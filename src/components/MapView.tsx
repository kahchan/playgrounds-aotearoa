...
function addSourcesAndLayers(
  ...
  const markerColor = darkColors ? '#9b20d0' : '#6510a0'
  ...
  for (const layer of ['clusters', 'markers', 'route-number']) {
  ...
  for (const layer of ['markers', 'route-number']) {
...
export default function MapView({
  ...
  useEffect(() => {
    ...
    map.on('style.load', () => {
      ...
      ;(map.getSource('playgrounds') as maplibregl.GeoJSONSource)
        ?.setData(toGeoJSON(playgroundsRef.current, routeOrderRef.current))
  ...
  useEffect(() => {
    ...
    const source = mapRef.current.getSource('playgrounds') as
      ...
      | undefined
    source?.setData(toGeoJSON(playgrounds, routeOrder))
  }, [playgrounds, routeOrder, mapLoaded])
  ...
  // Highlight the leg starting at the selected playground.
  useEffect(() => {
    if (!mapLoaded || !mapRef.current) return
    const map = mapRef.current
