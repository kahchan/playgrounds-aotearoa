...
interface ChunkResult {
  ...
  legStats: { distance: number; elevationGain: number; steps: RouteStep[] }[]
}
...
async function fetchChunk(
  ...
  const legStats: { distance: number; elevationGain: number; steps: RouteStep[] }[] = []
  for (let i = 0; i < wayPts.length - 1; i++) {
    ...
    legStats.push({ distance: orsDistance, elevationGain, steps })
  }
      ...
      .then((chunkResults) => {
        ...
        const features = chunkResults.flatMap(({ legs, legStats }) =>
          legs.map((legCoords, i) => ({
            ...
            properties: { legIndex: legIndex++, isMtb: false, ...legStats[i] },
          })),
        )
